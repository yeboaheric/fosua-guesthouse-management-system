...
from accounts.models import AuditLog, AttendanceRecord, DepositCollection, DepositTarget, Employee, EmployeeQualification, Expense, LeaveRequest, Notification, OwnerWithdrawal, PayrollRecord, Rota, RolePermission, TrainingRecord, UserAccessProfile
...
class SalesDepositsModuleTests(TestCase):
    ...
    def test_user_can_create_weekly_deposit_target(self):
        self.client.force_login(self.reception_user)
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Weekly sales deposit target created successfully.")
        target = DepositTarget.objects.get()
        self.assertEqual(str(target.target_amount), "500.00")
        self.assertEqual(target.week_start, week_start)
        self.assertEqual(target.week_end, week_end)
        self.assertEqual(target.created_by, self.reception_user)

    def test_collection_logs_against_one_week_and_updates_remaining(self):
        self.client.force_login(self.reception_user)
        week_start = timezone.localdate() - timedelta(days=timezone.localdate().weekday())
        ...
        collected_at = timezone.make_aware(datetime.combine(week_start + timedelta(days=2), time(hour=10)))

        response = self.client.post(
            reverse("sales-deposit-collection-create", args=[target.pk]),
            {
                "date_collected": timezone.localtime(collected_at).strftime("%Y-%m-%dT%H:%M"),
                "amount": "125.00",
                "collection_method": DepositCollection.CollectionMethod.MOBILE_MONEY,
                "collected_by": "Owner",
                "note": "Midweek collection",
            },
            follow=True,
        )

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Sales deposit collection logged against the weekly target.")
        collection = DepositCollection.objects.get()
        self.assertEqual(collection.deposit_target, target)
        self.assertEqual(collection.recorded_by, self.reception_user)
        self.assertEqual(str(target.remaining_balance), "375.00")
        self.assertContains(response, "GHS 375.00")

    def test_weekly_target_overdue_status_is_flagged_after_week_passes(self):
        self.client.force_login(self.admin_user)
        week_start = timezone.localdate() - timedelta(days=14 + timezone.localdate().weekday())
        target = DepositTarget.objects.create(
            week_start=week_start,
            week_end=week_start + timedelta(days=6),
        ...
        DepositCollection.objects.create(
            deposit_target=target,
            date_collected=timezone.make_aware(datetime.combine(week_start + timedelta(days=1), time(hour=9))),
            amount="100.00",
            collected_by="Owner",
            recorded_by=self.admin_user,
        )

        response = self.client.get(reverse("sales-deposits-center"))

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Overdue")
        self.assertContains(response, "GHS 200.00")

    def test_weekly_deposit_targets_export_selected_week(self):
        week_start = timezone.localdate() - timedelta(days=timezone.localdate().weekday())
        target = DepositTarget.objects.create(
            week_start=week_start,
            week_end=week_start + timedelta(days=6),
        ...
        response = self.client.get(
            reverse("sales-deposit-targets-export-xlsx"),
            {
                "target_start_date": week_start.isoformat(),
                "target_end_date": (week_start + timedelta(days=6)).isoformat(),
            },
        )
        ...
        workbook = load_workbook(BytesIO(response.content))
        sheet = workbook["Weekly Deposits"]
        self.assertEqual(sheet["A4"].value, "Week Range")
        self.assertEqual(sheet["B5"].value, 700)
        self.assertEqual(sheet["D5"].value, 250)
